...
export function MatchCard({ match }: { match: ProcessedMatch }) {
  ...
  const matchTitle = titleParts.length > 1 ? titleParts[1] : match.title;
            ...
            >
              {match.outcomes.map((outcome, idx) => {
                ...
                return (
                  <button
                    key={idx}
                    className="w-full h-[48px] rounded-lg bg-[#242b32] hover:bg-[#2f353c] border-b-[3px] border-[#121518] active:border-b-0 active:translate-y-[3px] text-white flex flex-col md:flex-row justify-center md:justify-between items-center px-4 transition-all duration-100"
                  >
